...
def _connect_spiders(graph: GraphS, v1, v2, hadamard=False):
    graph.add_edge((v1, v2), zx.EdgeType.HADAMARD if hadamard else zx.EdgeType.SIMPLE)
...
def clifford(qubits: Optional[int] = None, spiders: Optional[int] = None) -> GraphS:
    ...
    if qubits is None: qubits = random.randint(2, 5)
    if spiders is None: spiders = random.randint(5, 15)
    ...
    for i in range(qubits): _connect_spiders(g, inputs[i], input_zs[i])
    ...
    for i in range(qubits): _connect_spiders(g, output_zs[i], outputs[i])
