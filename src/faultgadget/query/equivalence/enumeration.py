...
def _smallest_size_iteration(g1_sig_nf: List[GF2], g2_sig_nf: List[GF2],
                             g1_sinks: int, g2_boundaries: int, g2_sinks: int, quiet: bool = True) -> Optional[int]:
    """
    ...
    for max_size in tqdm(range(1, len(g2_sig_nf_ints) + 1),
                        desc='Weight: ', initial=1, total=len(g2_sig_nf_ints), leave=False, disable=quiet):
        ...
        g2_new_detectable = []
        for last_it_int, atomic_sig_nf_int in tqdm(itertools.product(g2_last_new_detectable, g2_sig_nf_ints),
                                        desc='Signatures: ', total=len(g2_last_new_detectable) * len(g2_sig_nf_ints),
            ...
            if combined_sig >> g2_sinks not in g1_lookup:
                if not quiet: tqdm.write(f"{_format_sig(combined_sig, g2_boundaries, g2_sinks)} has no equivalent in g1, or it was not yet generated and thus has higher weight!")
