...
def _calculate_signature_normal_forms(signatures: Mapping[int, Signature], stabs: AugmentedStabilisers,
                                      boundaries_to_idx: Mapping[int, int], sink_to_idx: Mapping[int, int]) -> List[GF2]:
    signature_normal_forms: List[GF2] = []
    for sig in signatures.values():
        compiled_sig = sig.compile(boundaries_to_idx, sink_to_idx)
        signature_normal_forms.append(stabs.normalise_signature(compiled_sig))
...
def _construct_signatures(g: GraphS, stabilisers: Stabilisers, boundaries_to_idx: Mapping[int, int])\
    ...
    sig_nf = _calculate_signature_normal_forms(signatures, stabs, boundaries_to_idx, sink_id_to_idx)
...
def is_fault_equivalent(g1: GraphS, g2: GraphS, quiet: bool = True) -> bool:
    ...
    g1_stabs, g1_sig_nf, g1_num_sinks = _construct_signatures(g1, stabilisers, g1_boundaries_to_idx)
    if not quiet: print(f"Retrieved {len(g1_sig_nf)} signatures for g1!")

    if not quiet: print("Constructing signatures of g2...")
    g2_stabs, g2_sig_nf, g2_num_sinks = _construct_signatures(g2, stabilisers, g2_boundaries_to_idx)
